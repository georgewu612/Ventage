...
export default function PortfolioPage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      if (expRes.status === "fulfilled" && expRes.value.ok) setExposure(await expRes.value.json());
  ...
  const refresh = async () => {
    ...
    if (userId) await fetch(`${API_BASE_URL}/v1/portfolio/snapshot?user_id=${userId}`, { method: "POST" });
    setRefreshing(false);
  };
  ...
  const addHolding = async () => {
    ...
    try {
      const r = await fetch(`${API_BASE_URL}/v1/portfolio/upload`, {
        ...
        body: JSON.stringify({
          user_id: userId,
          holdings: [{ symbol: addSymbol.toUpperCase(), quantity: parseFloat(addQty), avg_cost: parseFloat(addCost) }],
        }),
      });
      if (!r.ok) throw new Error((await r.json()).detail);
      setShowAdd(false); setAddSymbol(""); setAddQty(""); setAddCost("");
      await loadAll();
    } catch (e: unknown) { setAddError(e instanceof Error ? e.message : "错误"); }
    setAddLoading(false);
  };

  const deleteHolding = async (symbol: string) => {
    if (!userId) return;
    await fetch(`${API_BASE_URL}/v1/portfolio/holding/${symbol}?user_id=${userId}`, { method: "DELETE" });
    await loadAll();
  };
  ...
  const importCsv = async () => {
    ...
    try {
      ...
      await loadAll();
    } catch (e: unknown) { setCsvError(e instanceof Error ? e.message : "解析失败"); }
    setCsvLoading(false);
  };
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mb-6 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">持仓管理 · 收益追踪 · 风险敞口</p>
            </div>
          ...
          {!loading && isEmpty && (
            <div className="rounded-2xl border border-white/10 bg-white/5 py-20 text-center">
              ...
              <p className="mt-1 text-sm text-gray-600">点击「添加持仓」手动录入，或「导入 CSV」批量导入</p>
          ...
          {!loading && summary && summary.holdings.length > 0 && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                {[
                  { label: "总市值", value: fmt(summary.total_value), sub: null },
                  { label: "总成本", value: fmt(summary.total_cost), sub: null },
                  { label: "总盈亏", value: fmt(summary.total_pnl), sub: `${summary.total_pnl_pct.toFixed(1)}%`, positive: summary.total_pnl >= 0 },
                  { label: "持仓数量", value: String(summary.position_count), sub: "只股票" },
                  ...
                  <div key={label} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    ...
                    {sub && <p className={`mt-0.5 text-xs ${positive === undefined ? "text-gray-500" : positive ? "text-emerald-600" : "text-red-600"}`}>{sub}</p>}
                  </div>
                ))}
              </div>
              ...
              {history.length > 1 && (
                <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                  <p className="mb-3 text-sm font-semibold text-gray-300">组合净值曲线（近 90 天）</p>
                      ...
                      <tbody className="divide-y divide-white/5">
                        {summary.holdings.map((h) => (
                          <tr key={h.symbol} className="hover:bg-white/[0.03]">
                            <td className="px-3 py-3 font-mono text-sm font-bold text-cyan-400">{h.symbol}</td>
                            <td className="px-3 py-3 text-sm text-gray-300">{h.quantity}</td>
                            <td className="px-3 py-3 text-sm text-gray-400">${h.avg_cost.toFixed(2)}</td>
                            <td className="px-3 py-3 text-sm text-white">${h.current_price.toFixed(2)}</td>
                            <td className="px-3 py-3 text-sm text-gray-300">{fmt(h.market_value)}</td>
                            <td className="px-3 py-3"><PnlBadge value={h.pnl} pct={h.pnl_pct} /></td>
                            <td className="px-3 py-3">
                              <div className="flex items-center gap-2">
                                <div className="h-1.5 w-16 overflow-hidden rounded-full bg-white/10">
                                  <div className="h-full bg-cyan-500" style={{ width: `${Math.min(h.weight, 100)}%` }} />
                                </div>
                                <span className="text-xs text-gray-500">{h.weight.toFixed(1)}%</span>
                              </div>
                            </td>
                            <td className="px-3 py-3">
                              <button onClick={() => deleteHolding(h.symbol)} className="text-gray-600 hover:text-red-400">
                  ...
                  {exposure && (
                    <>
                      <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                        <p className="mb-3 text-xs font-semibold tracking-wider text-gray-500 uppercase">行业分布</p>
                        {Object.entries(exposure.sectors)
                          .sort(([, a], [, b]) => b - a)
                          .map(([sec, wt]) => (
                            <div key={sec} className="mb-2">
                              <div className="mb-1 flex justify-between text-xs">
                                <span className="text-gray-400 truncate">{sec}</span>
                                <span className="text-gray-300 font-medium">{wt.toFixed(1)}%</span>
                              </div>
                              <div className="h-1 overflow-hidden rounded-full bg-white/10">
                                <div className="h-full bg-purple-500" style={{ width: `${Math.min(wt, 100)}%` }} />
                              </div>
                            </div>
                          ))}
                      </div>
                      ...
                      <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                        <p className="mb-3 text-xs font-semibold tracking-wider text-gray-500 uppercase">集中度风险</p>
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-400">前 5 大持仓</span>
                          <span className={`text-lg font-bold ${exposure.top5_concentration > 80 ? "text-red-400" : exposure.top5_concentration > 60 ? "text-amber-400" : "text-emerald-400"}`}>
                        ...
                        {exposure.top5_concentration > 70 && (
                          <div className="mt-3 flex items-start gap-2 rounded-lg bg-amber-500/10 p-2.5">
                            ...
                            <p className="text-xs text-amber-400">前 5 大持仓集中度较高，注意分散风险</p>
                          </div>
                        )}
                      </div>
                    </>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur">
          <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-slate-900 shadow-2xl">
            ...
            <div className="space-y-4 p-5">
              {[
                { label: "股票代码", value: addSymbol, set: setAddSymbol, placeholder: "NVDA", upper: true },
                { label: "持仓数量（股）", value: addQty, set: setAddQty, placeholder: "100" },
                { label: "平均成本（$/股）", value: addCost, set: setAddCost, placeholder: "450.00" },
            ...
            <div className="border-t border-white/10 p-5">
              <button onClick={addHolding} disabled={addLoading}
          ...
          <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-slate-900 shadow-2xl">
            <div className="flex items-center justify-between border-b border-white/10 p-5">
              <div>
                ...
                <p className="text-xs text-gray-500">支持 TD Ameritrade / IBKR 导出格式</p>
              </div>
              <button onClick={() => setShowCsv(false)} className="text-gray-500 hover:text-white"><X className="h-5 w-5" /></button>
            </div>
            <div className="space-y-4 p-5">
              <div>
                <p className="mb-2 text-xs text-gray-500">CSV 需包含列：<code className="text-cyan-400">symbol, quantity, avg_cost</code></p>
              ...
              <div>
                <label className="mb-1.5 block text-xs font-medium text-gray-400">或直接粘贴 CSV 内容</label>
            ...
            <div className="border-t border-white/10 p-5">
              <button onClick={importCsv} disabled={csvLoading || !csvText.trim()}
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-500 py-3 text-sm font-semibold text-white hover:bg-cyan-400 disabled:opacity-50">

...
| **新用户**          | 不知道看哪只股，先从「高置信做多机会」5 张卡片里挑感兴趣的点进去 |

---

## 🏗 页面结构（白话）

页面分两栏：**左侧两个主信号区**（占 2/3）+ **右侧个人面板（My Desk）**（占 1/3）。
...
- 🟢 风险偏好：可以正常出手
- 🟡 中性：保持一半仓位
- 🔴 避险 + VIX 极高 → **下面的所有信号都自动降级一档**，A 变 B，B 变 C

📸 **[建议截图：市场脉搏区块单独截图]**

**数据来源**：\`GET /v1/market/regime\` → 后端基于 VIX、SPY 200 日均线、行业广度计算
...
1. 看到感兴趣的代码 → 点卡片
2. 右侧弹出**信号详情侧栏**，展示完整分析
3. 决定要深入研究 → 点卡片里的代码跳到 \`单股工作台\` 做 6 维度评估
4. 还不满意？点底部 **「查看全部」** 去告警历史看更多

📸 **[建议截图：3 张做多机会卡片 + 信号详情弹出的样子]**
...
- VIX 进入"偏高"区间 → 顶部出现**橙色横幅警告**
- VIX 进入"极高"区间 → 出现**红色横幅**，建议暂停所有追多操作
...
**特别说明**：信号区会**实时更新**（30 秒轮询 + Supabase 实时推送），其他模块是页面打开时一次性加载。
...
1. **看市场脉搏**：今天是风险偏好还是避险？
   - 🟢 风险偏好 → 正常用 A 级信号
   - 🟡 中性 → 减半仓位
   - 🔴 避险 → 优先看风险台做空机会，或者休息一天

2. **扫高置信做多机会**：5 条机会里挑 1-2 个感兴趣的
   - 看综合分 → 看是期权 + 内部人多源融合还是单一信号源
   - 多源融合的 > 单源的

3. **点进去做深度分析**：跳到 \`单股工作台\`
   - 看蔡森形态识别 / Trading System v2 三引擎 / DCF 估值 / 质量评分
   - 综合判断后决定是否入场
...
2. 风险台出现 3+ 条空头信号 → 进一步确认大势走弱
3. 跳到 \`持仓\` 页看每只股的追踪止损是否触发
...
2. 点一张做多机会卡片，看信号详情长什么样
3. 把 5 只做多机会全部点一遍，对比哪只你最想买
4. 看市场脉搏的 AI 总结一句话，理解今天的"市场基调"
5. 检查右侧个人面板每个小块的数据是否符合你的实际账户
...
- 「查看全部」信号 → \`/dashboard/alerts\`
- 「查看持仓」 → \`/dashboard/portfolio\`
- 数据源 6 按钮 → 各 L4 数据情报页
...
为方便和英文 UI / 同事交流时对齐，列一份术语对照：

- **市场脉搏** = Market Pulse（顶部"市场状态 + VIX + 广度"那一条）
- **高置信做多机会** = High Conviction Setups（5 卡片做多信号）
- **风险台** = Risk Desk（做空 + 异常信号面板）
- **个人面板** = My Desk（右侧 6 小块，与"单股工作台 Stock Workbench"是两回事）
- **市场状态** = Regime（risk_on / neutral / risk_off）—— 保持英文行业术语
`,
